...
class NeuroSAT(object):
    ...
    def init_random_seeds(self):
        tf.random.set_seed(self.opts.tf_seed)
    ...
    def declare_parameters(self):
        ...
        with tf.compat.v1.variable_scope('params') as scope:
            self.L_init = tf.compat.v1.get_variable(name="L_init", initializer=tf.compat.v1.random_normal([1, self.opts.d]))
            self.C_init = tf.compat.v1.get_variable(name="C_init", initializer=tf.compat.v1.random_normal([1, self.opts.d]))
    ...
    def declare_placeholders(self):
        self.n_vars = tf.Variable(tf.ones(shape=[]), tf.int32, name='n_vars')
        self.n_lits = tf.Variable(tf.ones(shape=[]), tf.int32, name='n_lits')
        self.n_clauses = tf.Variable(tf.ones(shape=[]), tf.int32, name='n_clauses')
        ...
        # useful helpers
        self.n_batches = tf.cast(tf.shape(self.is_sat)[0], tf.float32)
    ...
    def while_body(self, i, L_state, C_state):
        ...
        LC_msgs = tf.compat.v1.sparse_tensor_dense_matmul(self.L_unpack, LC_pre_msgs, adjoint_a=True)

        with tf.compat.v1.variable_scope('C_update') as scope:
            _, C_state = self.C_update(tf.expand_dims(LC_msgs, axis=0), C_state[0])
            C_state = (C_state, 0)

        CL_pre_msgs = self.CL_msg.forward(C_state.h)
        CL_msgs = tf.compat.v1.sparse_tensor_dense_matmul(self.L_unpack, CL_pre_msgs)
    ...
    def build_optimizer(self):
        ...
        self.global_step = tf.compat.v1.get_variable("global_step", shape=[], initializer=tf.zeros_initializer(), trainable=False)
